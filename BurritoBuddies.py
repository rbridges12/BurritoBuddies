...
def find_bidirectional_matches_reverseorder(orders) -> list:
  ...
  while len(orders) >= 2:
    ...
    order = data[1]
    match_name, match_val = find_bidirectional_match(
        order, orders, match_value)
...
def get_best_match():
  ...
  for profile in match_profiles:
    ...
    matches.append((profile.get_name(), top_matches[0][0], round(
